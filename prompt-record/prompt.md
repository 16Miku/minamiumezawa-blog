...
> 本文档**完整保留**「梅澤美波博客归档」项目开发过程中用户下达的全部原始提示词（按时间顺序），
> 仅在每段原始提示词上方添加语境标题与简要说明，便于回溯。**原始提示词内容一字未改。**
...
| 序号 | 阶段 | 主题 |
|------|------|------|
| #1 | 需求提出 | 初始需求：复刻备份梅澤美波全部博客 |
| #2 | 方案探索 | 询问静态存档效果 |
| #3 | 方法学习 | vibe coding 技巧文章 |
| #4 | 原则确立 | 第一性原理 + 对抗式审查 |
| #5 | 合规确认 | 粉丝归档任务声明 |
| #6 | 指令设计 | 提出 /goal 指令 |
| #7 | 部署探索 | 询问 render/vercel 部署 |
| #8 | 部署确认 | 确认直接部署方案 |
| #9 | 完整指令 | 初版 /goal 完整要求 |
| #10 | 工程规范 | 更新 CLAUDE.md + git 规范 |
| #11 | 指令优化 | 要求生成详细 /goal 指令 |
| #12 | 指令体系 | 四阶段 /goal 指令体系 |
...
#13 整理目录结构：
```
删除外层被取代的废弃代码，并把 prompt-record 和 CLAUDE.md 迁移到 内层项目，整理更新并推送提交
```

#14 文档整理与内容验证：
```
1、请你整理 prompt.md这个提示词记录文档 。
2、更新项目开发文档和README.md
3、为本项目写一个github仓库的介绍信息
4、我想知道，当前复刻的梅泽美波博客的每一篇内容的文字和图片是否都是正确的和原版一致的？会不会因为我们项目是用AI Agent进行 vibe coding而出错？
```

---

> **整理说明**：本文档在保留全部原始提示词的前提下，仅添加了序号标题、目录表和语境说明。
> 所有 ``` 代码块内的内容均为用户原始输入，未作任何改动。
